...
author: "Team Name: Ethan, Izzat, Jamie, Brodie"
date: "`r Sys.Date()`"
...
plastic_data_all %>% select(region, name) %>% 
  count(region)
...
mean(plastic_data_all$total_mismanage_plastic)

max(plastic_data_all$total_mismanage_plastic)
```
...
percentage_waste <- plastic_data_all %>% 
                    mutate(pct_mismanaged_plastic_ocean = ( mismanaged_plastic_ocean /       mismanaged_plastic ) * 100) %>% 
  drop_na(pct_mismanaged_plastic_ocean)


median_waste <- percentage_waste %>% 
  group_by(region) %>% 
  select(pct_mismanaged_plastic_ocean) %>%
  summarise(median(pct_mismanaged_plastic_ocean))
